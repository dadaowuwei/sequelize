...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    return query.catch(err => {
      ...
      .then(queryResult => {
        if (!queryResult.rows && this.isUpsertQuery()) {
          // hack: in my version of postgres the queryResult has two results in an array structure
          // the first of which is a result for the upsert temp function create statement, and the
          // send is the result of the upsert, we are interested in the result of upsert which is
          // the result in the second position. -mdiplacido
          queryResult = _.last(queryResult);
        }

        let rows = queryResult.rows;
        let rowCount = queryResult.rowCount;
